  ...
  <div class = "row">
    <div class = "col-md-6">
      <div v-for = "note in notes" @click="current_note = note" :class=" { 'note': true, 'selected': current_note == note }">
        ...
        <div v-if="note.isEditMode">
          <input type="text" class="font-control" v-model="note.name" @keydown.enter="note.isEditMode = false">
        </div>
        <div>
    ...
    <template v-if="current_note != null">
          <div v-for="task in current_note.tasks" class="task">
            ...
            <div v-if="task.isEditMode">
              <input type="text" class="font-control" v-model="task.name" @keydown.enter="task.isEditMode = false">
            </div>
            <div>
